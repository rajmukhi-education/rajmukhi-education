...
if(req.method==='GET'&&p==='/api/courses')return send(res,200,db.courses);
...
if(req.method==='POST'&&p==='/api/progress'){const s=requireUser(req,res);if(!s)return;try{const b=await readBody(req);const courseId=String(b.course_id||'');const lessonId=String(b.lesson_id||'');if(!courseId||!lessonId)return send(res,400,{error:'course_id and lesson_id are required'});const lesson=db.lessons.find(l=>String(l.id)===lessonId&&String(l.course_id)===courseId);if(!lesson)return send(res,404,{error:'lesson not found for this course'});let x=db.progress.find(x=>x.user_id===s.user_id&&String(x.lesson_id)===lessonId);if(x){x.course_id=courseId;x.completed=!!b.completed}else{x={id:id(),user_id:s.user_id,course_id:courseId,lesson_id:lessonId,completed:!!b.completed,updated_at:new Date().toISOString()};db.progress.push(x)}x.updated_at=new Date().toISOString();saveDB();return send(res,200,{ok:true,...x})}catch{return send(res,400,{error:'invalid progress data'})}}
...
if(req.method==='GET'&&p==='/api/results'){const s=requireUser(req,res);if(!s)return;const all=s.role==='admin'?db.results:db.results.filter(r=>r.student_id===s.user_id);return send(res,200,all)}
...
if(req.method==='PUT'&&p.startsWith('/api/admin/tests/')){if(!requireAdmin(req,res))return;const x=db.tests.find(x=>x.id===p.split('/').pop());if(!x)return send(res,404,{error:'test not found'});const b=await readBody(req);if(b.title!==undefined)x.title=String(b.title);if(b.course_id!==undefined)x.course_id=String(b.course_id);if(Array.isArray(b.questions))x.questions=b.questions.map((q,i)=>({id:i+1,question:String(q.question||''),options:Array.isArray(q.options)?q.options.map(String):[],answer:Number(q.answer||0)}));saveDB();return send(res,200,x)}
if(req.method==='POST'&&p==='/api/admin/notes'){if(!requireAdmin(req,res))return;const b=await readBody(req);if(!b.title)return send(res,400,{error:'title is required'});const x={id:id(),title:String(b.title),description:b.description||'',file_url:b.file_url||'',created_at:new Date().toISOString()};db.notes.push(x);saveDB();return send(res,201,x)}
...
if(req.method==='POST'&&p==='/api/admin/tests'){if(!requireAdmin(req,res))return;const b=await readBody(req);if(!b.title||!Array.isArray(b.questions)||!b.questions.length)return send(res,400,{error:'title and questions are required'});const qs=b.questions.map((q,i)=>({id:i+1,question:String(q.question),options:Array.isArray(q.options)?q.options.map(String):[],answer:Number(q.answer||0)}));const x={id:id(),title:String(b.title),course_id:b.course_id||'',questions:qs,created_at:new Date().toISOString()};db.tests.push(x);saveDB();return send(res,201,x)}
...
if(req.method==='GET'&&p==='/api/dashboard/student'){const s=requireUser(req,res);if(!s)return;const enrolled=db.enrollments.filter(x=>x.user_id===s.user_id);const progress=db.progress.filter(x=>x.user_id===s.user_id);const results=db.results.filter(x=>x.student_id===s.user_id);const courseSummary=enrolled.map(e=>{const c=db.courses.find(x=>x.id===e.course_id)||{};const lessons=db.lessons.filter(x=>x.course_id===e.course_id).sort((a,b)=>(a.order||0)-(b.order||0));const done=progress.filter(x=>x.course_id===e.course_id&&x.completed).length;const next=lessons.find(l=>!progress.some(x=>x.lesson_id===l.id&&x.completed));return {course_id:e.course_id,title:c.title||'Course',completed:done,total:lessons.length,percent:lessons.length?Math.round(done/lessons.length*100):0,nextLesson:next?{id:next.id,title:next.title}:null}});const completedLessons=progress.filter(x=>x.completed).length;const avgScore=results.length?Math.round(results.reduce((n,r)=>n+(r.total?100*r.score/r.total:0),0)/results.length):0;const days=new Set(results.map(r=>new Date(r.created_at).toISOString().slice(0,10)));let streak=0;for(let i=0;i<365;i++){const d=new Date(Date.now()-i*86400000).toISOString().slice(0,10);if(days.has(d))streak++;else if(i>0)break}const notifications=db.notices.slice().sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)).slice(0,5);const certificates=courseSummary.filter(c=>c.total>0&&c.percent===100).map(c=>({course_id:c.course_id,title:c.title,eligible:true}));return send(res,200,{user_id:s.user_id,enrolled:enrolled.length,completedLessons,attempts:results.length,avgScore,streak,courseSummary,notifications,certificates})}
