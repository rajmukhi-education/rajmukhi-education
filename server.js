...
if(req.method==='DELETE'&&p.match(/^\/api\/admin\/uploads\/[^/]+$/)){if(!requireAdmin(req,res))return;const uid=decodeURIComponent(p.split('/').pop());const item=db.uploads.find(x=>x.id===uid);if(!item)return send(res,404,{error:'upload not found'});const fileName=path.basename(String(item.url||''));const fp=path.join(UPLOAD_DIR,fileName);try{if(fs.existsSync(fp))fs.unlinkSync(fp)}catch{}db.uploads=db.uploads.filter(x=>x.id!==uid);saveDB();return send(res,200,{ok:true,deleted:uid})}
