...
if(req.method==='POST'&&p==='/api/auth/logout'){const s=auth(req);if(s){db.sessions=db.sessions.filter(x=>x.token!==s.token);saveDB()}return send(res,200,{ok:true})}
...
if(req.method==='POST'&&p==='/api/progress'){const s=requireUser(req,res);if(!s)return;const b=await readBody(req);if(!b.course_id||!b.lesson_id)return send(res,400,{error:'course_id and lesson_id are required'});let x=db.progress.find(x=>x.user_id===s.user_id&&x.lesson_id===b.lesson_id);if(x)x.completed=!!b.completed;else{x={id:id(),user_id:s.user_id,course_id:b.course_id,lesson_id:b.lesson_id,completed:!!b.completed,updated_at:new Date().toISOString()};db.progress.push(x)}x.updated_at=new Date().toISOString();saveDB();return send(res,200,x)}
